...
class LedgerTests(unittest.TestCase):
    ...
    def test_basal_clamp_wiring_is_exact_when_generated(self) -> None:
        ...
        self.assertEqual(transduction["candidate_edges"], 7536)
        self.assertEqual(transduction["free_continuous_parameters"], 7536)
        self.assertEqual(transduction["resolved_motor_groups"], 368)
        self.assertEqual(transduction["unresolved_motor_groups"], 73)
        self.assertEqual(transduction["resolved_motor_neurons"], 722)
        self.assertEqual(transduction["unresolved_motor_neurons"], 93)
        self.assertEqual(transduction["covered_actuators"], 91)
        self.assertEqual(transduction["uncovered_actuators"], 11)
